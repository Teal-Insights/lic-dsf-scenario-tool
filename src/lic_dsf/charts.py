...
def comparison_summary(comparison):
    ...
    if 'format' in comparison and version not in ('lic-dsf-comparison-v1', 'lic-dsf-comparison-v2'):
        raise ValueError('Unsupported comparison format.')
    if version == 'lic-dsf-comparison-v2' and 'chart_context' not in comparison:
...
def _footnote(summary, metric):
    ...
    return source+' Sampled points; connecting lines are visual guides. Gaps preserve missing/error values.'
...
def _title(summary,metric,briefing):
    ...
    return f'{label} is {abs(diff):.1f} pp {direction} in {row["year"]}'
...
def _panel(ax,summary,metric,view,large=False,heading=True):
    ...
    if heading:
        ax.set_title(textwrap.fill(_title(summary,metric,briefing),65 if large else 40),loc='left',fontsize=17 if large else 12,fontweight='bold',pad=25 if briefing else 12,color=COLORS['ink'])
    ...
    colors=[COLORS['cyan'],'#6B7280','#8E5EA2','#A56122'];styles=['--','-.',':',(0,(5,1,1,1))];markers=['s','D','^','v']
    focus=_focus(summary,rows);focus_label=focus[0] if focus else None
    for i,label in enumerate(summary['labels']):
        color=COLORS['cyan'] if briefing and label==focus_label else colors[i]
        if briefing and label!=focus_label:color=COLORS['gray'] if label==summary['comparator'] else '#8A8A8A'
    ...
    for label,values,color,style,marker in series:
        ...
        if briefing:
            ...
            if valid:
                ...
                end_label=f'{label}: {end_value:.2f}' if end_year==max(years) else f'{label}: {end_value:.2f} ({end_year})'
                endings.append([end_value,end_label,color,end_year])
    threshold=rows[0]['threshold']
    if _numeric(threshold):
        if briefing:
            # Keep the benchmark out of the endpoint-label column: labels cannot
            # appear to name a benchmark at their displaced text coordinates.
            ax.plot([min(years),max(years)],[threshold,threshold],color='#444444',ls=(0,(5,4)),lw=1.2)
        else:ax.axhline(threshold,color='#444444',ls=(0,(5,4)),lw=1.2,label='Benchmark' if metric.startswith('public_') else 'Threshold')
    ...
    if briefing:
        ...
        if focus and not any(e['status']=='Review required' for e in summary['evidence']):
            ...
            annotation=ax.text(.01,.96,'\n'.join(wrapped)+f'\n{row["year"]}: {diff:+.2f} percentage points',transform=ax.transAxes,va='top',fontproperties=font,parse_math=False,usetex=False,color=COLORS['navy'],bbox={'facecolor':'white','edgecolor':'none','alpha':.85})
            annotation.set_gid('comparison-annotation')
    else:
        from matplotlib.font_manager import FontProperties
        columns=2 if len(series)>3 else 1
        font=FontProperties(family='DejaVu Sans',size=size-1)
        handles,labels=ax.get_legend_handles_labels()
        width=ax.get_window_extent().width/columns-(size-1)*5*ax.figure.dpi/72
        labels=['\n'.join(_header_lines(label,ax.figure.canvas.get_renderer(),font,width)) for label in labels]
        ax.legend(handles,labels,loc='best',prop=font,frameon=False,ncol=columns)
    ax.text(0,-.24 if large else -.28,textwrap.fill(_footnote(summary,metric),120 if large else 85),transform=ax.transAxes,fontsize=9 if large else 7.7,color=COLORS['gray'],va='top')
...
def _dot_panel(ax,summary,metric):
    ...
    for i,(label,value,color,marker) in enumerate(entries):
        ...
        if _numeric(value):
            ...
            diff=('0.00' if difference==0 else '≈0.00' if abs(difference)<.005 else f'{difference:+.2f}') if difference is not None else 'Unavailable'
...
def _header(fig, summary, view, *, detail=False):
    ...
    if context['label'] is not None:line(context['label'],12,COLORS['navy'],'bold',context_label=True)
    line('Workbook SHA-256: '+context['workbook_sha256'][:12]+'…',10,COLORS['gray'])
    line('Selected comparator: '+summary['comparator']+' | Reference baseline retained',11,COLORS['gray'])
    ...
    if view=='briefing' and not detail:
        line('Sampled-year comparisons | Δ: difference versus the selected comparator',10,COLORS['gray'])
        line('Each indicator uses its own zoomed scale; compare the numeric differences, not lengths across panels.',9,COLORS['gray'])
...
def render_comparison(comparison, view='standard', format='png'):
    ...
    with _RENDER_LOCK, matplotlib.rc_context({'font.family':'DejaVu Sans','text.usetex':False,'pdf.fonttype':42,'savefig.facecolor':'white'}):
        ...
        fig.text(.075,.038,textwrap.fill('Source: saved workbook stress tables. '+SCOPE+' '+summary['interpretation'],160),fontsize=9,color=COLORS['gray'],va='bottom')
