...
This is an experimental preview. Bounded Mac and Windows Server evidence exists for earlier builds; final alpha packages need their own checks. No WCAG conformance, complete assistive-technology acceptance or universal cross-platform support is claimed.
...
| macOS | Earlier installed browser/calculation/packet checks passed on Apple Silicon. The new macOS14+ arm64 bundle has separate relocation and worker checks; final downloaded Finder launch remains pending |
| Windows | Earlier package tested on Windows Server2022x64 as Administrator, without Excel: launch, upload, two calculations, charts, packet, scenario re-import and restart. Final alpha and managed Windows11 ordinary-user acceptance remain pending |
